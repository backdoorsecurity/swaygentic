...
public class RawRenderer implements Renderer {
    ...
    void render(InputStream in, Bitmap destination, int x, int y, int width, int height) throws VncException {
        try {
            int sx = x;
            int sy = y;
            for (int i = 0; i < width * height; i++) {
                Pixel pixel = pixelDecoder.decode(in, pixelFormat);
                destination.setPixel(sx, sy, BitmapColors.rgb(pixel.getRed(), pixel.getGreen(), pixel.getBlue()));
                sx++;
                if (sx == x + width) {
                    sx = x;
                    sy++;
